...
// Wrap a token caption so hovering reveals the full per-request usage the
// provider volunteered. No-op when there are no extras to show.
Widget _withUsageTip(AiUsage usage, Widget child) {
...
class _ChangesPageState extends State<ChangesPage> {
  ...
  void _syncDraftFromStatus(RepositoryStatus status) {
    ...
    final multiTabRestore = restoredTabs != null && restoredTabs.length > 1;
    if (multiTabRestore) {
      _tabs
        ..clear()
        ..addAll(restoredTabs);
    ...
    if (restored != null) {
      // `restored` is the departed context's *active-tab* set. Replay it
      // only in the single-tab case. On a multi-tab restore each tab
      // already carries its own paths from `restoredTabs`, and
      // `_includedPaths` now resolves to tab 0 (active index forced to 0
      // above) — overwriting it here would clobber tab 0's real contents
      // with the departed active tab's set and duplicate those paths
      // across two tabs, silently changing the committed file set.
      if (!multiTabRestore) {
        _includedPaths
          ..clear()
          ..addAll(restored);
