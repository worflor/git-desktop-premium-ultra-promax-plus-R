...
class BondBackend implements CollaborationBackend {
  ...
  /// sessions are created by [_ensurePeerSession] via the transport.
  Future<void> attachPeerSession({
    ...
    peer.start();
    peer.refAdverts.listen((advert) {
      runtime.lastAdverts[_hex(peer.session.remotePubkey)] = advert;
  ...
  Future<GitResult<SyncData>> fetch(
    ...
    for (final peer in runtime.peers.values) {
      ...
      try {
        ...
        if (advert != null) {
          await _writeBondRefs(runtime, hex, advert);
          fetchedRefs += advert.refs.length;
          buffer.writeln(
            'bond: updated ${advert.refs.length} ref(s) from ${_shortHex(peer.session.remotePubkey)}',
          );
        }
      } catch (e) {
        buffer.writeln('bond: fetch from ${_shortHex(peer.session.remotePubkey)} failed: $e');
      }
    }
  ...
  Future<GitResult<SyncData>> sync(
    ...
    if (!pushed.ok) {
      return GitResult.ok(SyncData(
        ...
        remote: fetched.data!.remote,
        output:
            '${fetched.data!.output}\nbond: push skipped: ${pushed.error}',
      ));
    }
    return GitResult.ok(SyncData(
      operation: 'sync',
  ...
  /// matching objectPack response. Times out after 30s — caller wraps
  /// in try/catch and reports per-peer failure.
  Future<void> _requestPack({
    ...
    final pack = await peer.packfiles
        .first
  ...
  Future<void> _persistLamport(_RepoRuntime runtime) async {
    ...
    await file.writeAsString(
      jsonEncode(<String, dynamic>{'self': runtime.lamportClock}),
      flush: true,
    );
  }

  /// Writes refs from an accepted advert into the `refs/bond/<signer>/`
  /// namespace. Peers read each other's work by checking out refs in
  /// this namespace; a later merge or rebase moves them into the
  /// user's own branches. Uses plain file writes into .git/refs rather
  /// than `git update-ref` per-call to keep the hot-path cheap.
  Future<void> _writeBondRefs(
    _RepoRuntime runtime,
    String signerHex,
    RefAdvert advert,
  ) async {
    for (final entry in advert.refs.entries) {
      ...
      if (proc.exitCode != 0) {
        // Non-fatal: ref write failures are logged via caller.
      }
    }
  }
  ...
  // Silence unused-field warning: retained for future use by
  // higher-level services (repo-config resolver).
  // ignore: unused_element
  String _repoConfigPath(String repoPath) =>
      p.join(repoPath, '.git', 'manifold', 'bond', 'config.json');
}
