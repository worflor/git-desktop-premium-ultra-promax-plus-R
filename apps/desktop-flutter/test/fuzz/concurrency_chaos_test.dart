...
// (max 6, adaptive), not a mutex. Index-WRITING calls are serialized per
// repo (`_withRepoIndexWriteLock` — git's own index.lock never made the
// whole read-modify-write atomic, so overlap lost updates); everything
// else (reads, ref writers) still overlaps freely. This file races real
// operations on scratch repos with seeded jitter (test/support/chaos.dart)
// and asserts invariant oracles.
//
...
//      DIFFERENT files both stage. Fixed TWICE at successively deeper roots:
//      the index.lock collision is retried away (C2, 2026-07-10), and the
//      residual lost-update — git reads the index before locking it, so
//      overlapping writers commit stale snapshots — is now unrepresentable
//      via the per-repo index-write lock (2026-07-13, see law D3 in
//      deterministic_interleaving_test.dart).
...
const bool _knownFindingCreateCasSkip =
    false; // law 3 (C1) — zero-oid CAS on create
const bool _knownFindingStagingInterleaveSkip =
    false; // law 5 (C2) — gated retry
...
// without amputating an honest heavy run.
final Timeout _chaosTimeout = fuzzTimeout(const Duration(minutes: 4));
...
void main() {
  ...
  group('law 1 — applyPatch participates in the watcher-pause contract', () {
    test('an in-flight applyPatch raises gitMutationsInFlight and notifies, so '
      ...
      try {
        ...
        expect(
          ...
          greaterThan(0),
          reason:
              'a gated mutating git call must raise '
        ...
        expect(
          ...
          greaterThan(0),
          reason:
              'applyPatch must raise mutations-in-flight mid-call so the '
              'watcher pauses instead of racing the staging write',
        );
        expect(
          gitMutationsInFlight,
          0,
        ...
        expect(
          ...
          2,
          reason:
              'applyPatch must notify once on the +1 bump and once on the '
  ...
  group('law 2 — concurrent addComment never loses a comment', () {
    test(
      'N concurrent addComment: thread == successes, ref parses, ≥1 wins',
      () async {
          ...
          check: (rng) async {
            ...
            try {
              ...
              for (var rep = 0; rep < chaosReps(); rep++) {
                ...
                final successes = valuesOf(outcomes).where((ok) => ok).length;
                expect(
                  successes,
                ...
                expect(
                  ...
                  isTrue,
                  reason:
                      'PR ref did not parse after the race: '
                      '${reread.error}',
                );
                expect(reread.data, isNotNull);
                expect(
                  reread.data!.thread.length,
                  successes,
                  reason:
                      'lost update: comments present '
        ...
        try {
          ...
          for (var rep = 0; rep < chaosReps(); rep++) {
            ...
            expect(
              ...
              1,
              reason:
                  'two concurrent create()s must leave exactly one winner: '
  ...
  group('law 4 — concurrent id allocation is distinct and monotonic', () {
    test(
      'N concurrent allocSequentialId → ids == 1..N (in-process)',
      () async {
        await forAllAsync<Rng>(
          _identityRng,
          count: fuzzScale(),
            ...
            try {
              for (var rep = 0; rep < chaosReps(); rep++) {
                ...
                expect(
                  ...
                  isTrue,
                  reason:
                      'an in-process alloc lost its local CAS: '
                ...
                expect(
                  ids,
                  equals(List<int>.generate(n, (i) => i + 1)),
                  reason:
                      'ids were not the distinct, monotonic range 1..$n '
  ...
  group('law 5 — concurrent applyFileStaging on different files', () {
    test(
      'both files stage with the expected content; index never corrupts',
      () async {
          ...
          for (var rep = 0; rep < chaosReps() * 2; rep++) {
            ...
            for (final r in valuesOf(outcomes)) {
              expect(
                ...
                isTrue,
                reason:
                    'applyFileStaging is a checked `git reset -q HEAD -- '
                    '<file>` followed by `applyPatch --cached`. In-process '
                    'callers serialize on the per-repo index-write lock and '
                    'cross-process index.lock contention is retried away, so '
                    'a concurrent invocation must neither leak a lock error '
                    'nor lose the other op\'s update: ${r.error}',
              );
            }
  ...
  group('law 6 — applyPatch racing a gated git add', () {
    test(
      'repo stays sane, no torn index; index.lock leaks are surfaced',
      () async {
        await forAllAsync<Rng>(
          _identityRng,
          count: fuzzScale(),
          ...
          check: (rng) async {
            ...
            try {
              for (var rep = 0; rep < chaosReps(); rep++) {
                ...
                final patch = _newFileDiff(appliedPath, [
                  'applied content $rep',
                ]);
                ...
                if (add.exitCode == 0) {
                  ...
                  expect(showAdded.stdout.toString(), 'added content $rep\n');
                }
              }
            } finally {
              await repo.dispose();
  ...
  group('law 7 — the subprocess semaphore caps concurrency', () {
    test(
      '20 concurrent reads never exceed gitSubprocessMaxConcurrency',
      () async {
        ...
        try {
          ...
          for (var rep = 0; rep < chaosReps(); rep++) {
            ...
            final thunks = <Future<ProcessResult> Function()>[
              for (var i = 0; i < 20; i++)
                () => repo.git(
                  i.isEven ? ['rev-parse', 'HEAD'] : ['status', '--porcelain'],
                ),
            ];
            final outcomes = await raceAll<ProcessResult>(
              thunks,
              rng,
              maxJitterMs: 0,
            );
            ...
            expect(
              peak,
              lessThanOrEqualTo(gitSubprocessMaxConcurrency),
              reason:
                  'peak concurrency $peak exceeded the ceiling '
            ...
            expect(
              ...
              reason:
                  'the burst never overlapped (peak $peak) — the ceiling '
    ...
    test(
      ...
      () async {
        ...
        for (var rep = 0; rep < chaosReps(); rep++) {
          ...
          ShadowCouplingCacheData contribution(String from, String to) =>
          ...
          final outcomes = await raceAll<void>(<Future<void> Function()>[
            () => cycle('a.dart', 'b.dart'),
            () => cycle('c.dart', 'd.dart'),
          ], rng);
          ...
          expect(
            finalCache!.jaccardEdges['a.dart']?['b.dart'],
            1.0,
            reason:
                'the a→b edge is missing — ShadowCouplingCache.save must '
                'serialize as save-as-merge per repo key (a lost update means '
                'the per-key save lock regressed)',
          );
          expect(
            finalCache.jaccardEdges['c.dart']?['d.dart'],
            1.0,
  ...
  group('law 9 — concurrent disjoint write/stage halves commute', () {
    test(
      'both halves stage disjoint files; the sequential commit has all',
      () async {
        await forAllAsync<Rng>(
          _identityRng,
          count: fuzzScale(),
              ...
              for (var rep = 0; rep < chaosReps(); rep++) {
                ...
                for (final (path, content) in [...filesA, ...filesB]) {
                  ...
                  expect(
                    ...
                    content,
                    reason:
                        'content mismatch for $path after concurrent '
                        'disjoint staging',
                  );
                }
              }
