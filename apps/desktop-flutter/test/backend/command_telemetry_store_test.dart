...
//
// Hermetic when run with GDPU_DATA_DIR pointed at a temp dir.
...
void main() {
  test('append after a torn (newline-less) last line preserves the new sample',
      () async {
    final dir = await StoragePaths.gdpuDataDir();
    await dir.create(recursive: true);
    final file =
        File('${dir.path}${Platform.pathSeparator}command_telemetry.jsonl');
    ...
    expect(commands.contains('after_torn'), isTrue,
        reason: 'torn last line swallowed the new sample');
    expect(commands.contains('clean_before'), isTrue,
